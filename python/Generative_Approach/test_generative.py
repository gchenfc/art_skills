...
class InputParameters:
    ...
    def shape_param(self, num_c):
        """
        Prompt the user for the shape parameter (Ac)
        used in intermediate parameterization
        """
        for i in range(num_c):
...
class StrokeGenerator:
    """Generates a single strokes."""

    # Define mu and sigma as described below, above their functions.
    # This comes from "Dijoua08EPM" paper sigma, according to Berio17, is the
    # response time in a logarithmic time scale
    @staticmethod
    def sigma(Ac: float):
        """Use the shape parameter to estimate the log response time, sigma"""
        return math.sqrt(-math.log(1 - Ac))  # assume ln, could be log10

    # mu, according to Berio17, is the stroke delay in a logarithmic time scale
    @staticmethod
    def mu(sigma, T: float):
        """Use sigmas and period to estimate the stroke delay, mu"""
        # again assuming ln
        return 3*sigma - np.log((-1 + np.exp(6*sigma))/T)
    ...
    def weight(self, tstep, T, dthetas, sigma, mu):
        ...
        for i in range(len(v_length)):
            ...
            for t in range(1, steps+1):  # assign weight to each step
                time = t*tstep
                # uses ln as assumed before, based on Berio eqtn
                z = (np.log(time - 0) - mu[i])/(sigma[i]*np.sqrt(2))
    ...
    def blah(self, d, angle):
        ...
        """
        steps = round(T[i]/time_step)
        theta_0 = angle_vec[i] + (np.pi + deltas[i])/2
        for t in range(0, steps-1):
            if norm(deltas[i]) > (1*10**(-10)):
                dx = D_vec[i]*((np.cos(theta_0 - deltas[i]*weights[i]
                                [t]) - np.cos(theta_0))*(2*np.sin(deltas[i]/2))**-1)
                dy = D_vec[i]*((np.sin(theta_0 - deltas[i]*weights[i]
                                [t]) - np.sin(theta_0))*(2*np.sin(deltas[i]/2))**-1)
            else:
    ...
    def displacement(self, time_step, T, dthetas, deltas, weights):
        """Generate stroke by weighted displacements at time (t)

        Args:
            time_step ([type]): 1 / sample rate for the stroke
            T ([type]): periods for the strokes
            dthetas ([type]): [description]
            deltas ([type]): [description]
            weights ([type]): [description]
        """
        D_vec = dthetas[:, 0]
        angle_vec = dthetas[:, 1]
        # TODO(frank): use a list comprehension
        # def blah(d, angle)
        # d_list = [self.blah(d,angle) for (d,angle) in dthetas]
        d_list = []
        d = []
    ...
    # this function currently only works for a single defined stroke.
    # Second stroke points are calculated from first stroke origin
...
class TrajectoryGenerator:
    ...
    def mu(self, sigs, Ts):
        ...
        for i in range(len(sigs)):
            # again assuming ln
            u[i] = 3*sigs[i] - np.log((-1 + np.exp(6*sigs[i]))/Ts[i])
        return u
...
class ArtSkills:
    ...
    def capture_input(self):
        ...
        parameter = np.array(
            [param.get_input(timestep_str), param.get_input(numpoints_str)])
        print(parameter)
...
class TestStrokeGenerator(unittest.TestCase):
    ...
    def test_mu(self):
        ...
        sigma, T = 0.3, 0.7
        # regression
        self.assertEqual(StrokeGenerator.mu(sigma, T), -1.0759933757620979)
...
#         """Test the method that generates the points."""
#         expected = np.zeros((5, 2))
#         actual = self.trajectory_generator.points()
...
class Test_All(unittest.TestCase):
    ...
    # third target point, as user input TODO: get this to work with xi < xi-1. yi < yi-1 works iff xi > xi-1
    string_of_pts_3 = '10,4'
    # central angle of the circular arc shape of curve 1 (rad)
    delta1 = '0.9944'
    # central angle of the circular arc shape of curve 2 (rad)
    delta2 = '0.3236'
    # shape parameter that defines skewedness of the lognormal (Berio17euro...)(Plamondon2003)
    Ac1 = '0.1'
    # shape parameter that defines skewedness of the lognormal (Berio17euro...)(Plamondon2003)
    Ac2 = '0.1'
    ...
    @ unittest.skip("move to notebook")
    @ patch('builtins.input', side_effect=[time_step, num_of_pts,
                                           string_of_pts_1, string_of_pts_2, string_of_pts_3,
                                           delta1, delta2, Ac1, Ac2, T1, T2])
    def test_input_sequence(self, mock_input):
        ...
        setup_param = np.array([param.get_input(timestep_str), param.get_input(
            numpoints_str)])  # get timestep and number of target points
        # get point string
        points = np.array(
            [param.get_input(point_str), param.get_input(point_str)])

        np.testing.assert_array_equal(setup_param, [0.01, 3])
        np.testing.assert_array_equal(points, [[0, 0], [6, 9], [10, 4]])
        # np.testing.assert_array_equal(
        #     curve_param, [[0.9944, 0.3236], [0.1, 0.1], [0.7, 0.8]])
...
#         gen = TrajectoryGenerator() # call class
