# %% [markdown]
# Try adding cnn to input embedding to give explicit smoothing reasoning
...
dataset_path = "data/gml_by_stroke_PRESERVE_ASPECT_CENTERED_021000.zarr"
# dataset_path = "data/gml_by_stroke_PRESERVE_ASPECT_CENTERED_030000.zarr"
...
# utils.drawing_lims = dict(x=(0, 1), y=(0, 1))
...
# %%
n_emb = 768
InputEmbedding = nn.Sequential(Transpose(1, 2), nn.Conv1d(3, 15, 5, padding=2),
                               nn.ReLU(), nn.Conv1d(15, n_emb, 5, padding=2),
...
network_kwargs = dict(
    ...
    augment_input_embedding=InputEmbedding)
...
noised_action = torch.randn((1, PRED_HORIZON, PREDICTION_DIM))
diffusion_iter = torch.zeros((1, ), dtype=torch.long)

# compute
noise = noise_pred_net(sample=noised_action,
                       timestep=diffusion_iter,
                       global_cond=None)

# check denoising
denoised_action = noised_action - noise
print("Ok!", "noise.shape:", noise.shape, "denoised_action.shape:",
      denoised_action.shape)
...
# num_epochs = 5 * 625 // len(dataloader) + 1
# num_epochs = 5 * 625 // len(dataloader) + 1
num_epochs = NUM_TRAINING_STEPS // len(dataloader) + 1
# num_epochs = 500 // len(dataloader) + 1
...
#%% Save temporary
json.dump(network_kwargs,
          open(f'{writer.log_dir}/network_kwargs.json', 'w'),
          default=repr,
          indent=2)
...
try:
        ...
        for epoch_idx in tglobal:
            ...
            with tqdm(dataloader, desc='Batch') as tdataloader:
                ...
                for batch_n in tdataloader:
                    ...
                    x = torch.cat([obs_n, action_n[..., -1:]], dim=-1)
                    x = x.repeat(TIMESTEP_REPEATS_PER_BATCH, 1, 1)
                    B = x.shape[0]
                    global_cond = None
                    ...
                    if lr_scheduler.last_epoch % 1000 == 0:
                        save_checkpoint(
                            f'{writer.log_dir}/ema_noise_pred_net_{lr_scheduler.last_epoch}.pth'
                        )
    ...
    all_losses.extend(epoch_loss)
    pass
writer.add_text('network_kwargs',
                json.dumps(network_kwargs, default=repr, indent=2))
writer.close()

ema_noise_pred_net = save_checkpoint(
...
for b in range(x_out.shape[0]):
    fig, axes = utils.plot_result(dataset,
                                  ...
                                  clean=True,
                                  axes=axes,
                                  traj_line_kwargs=dict(alpha=0.5))
fig.suptitle(
...
plt.savefig(f'{writer.log_dir}/output_example_{date}.svg')
np.savez(f'{writer.log_dir}/output_example_{date}.npz',
         x_out=x_out,
         x_train=x_)
...
for b in range(x_out.shape[0]):
    fig, axes = utils.plot_result(dataset,
                                  ...
                                  clean=True,
                                  axes=axes,
                                  traj_line_kwargs=dict(alpha=0.7))
fig.suptitle(
    f'{network_kwargs["n_layer"]} layers, {network_kwargs["n_head"]} heads, {network_kwargs["n_emb"]} emb',
    fontsize=24)
