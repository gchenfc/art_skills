...
class ExpressionSlnFactor
 ...
 public:
  ...
  typedef typename boost::shared_ptr<ExpressionSlnFactor> shared_ptr;
  typedef ExpressionSlnFactor This;
 ...
 public:
   ...
   */
  ExpressionSlnFactor(gtsam::Key parameters_key, gtsam::Key p0_key, double t,
  ...
  Vector evaluateError(
    ...
    auto predict = [this](const gtsam::Vector6_& parameters, const gtsam::Vector2_& p0) {
      SlnStrokeExpression stroke(p0, parameters);
      return stroke.position<50>(t_);
    };
    const gtsam::Vector2_ predicted_xy = predict(parameters, p0);
    ...
    return predicted_xy.value(gtsam::Values()) - xy_;
  }
